const App = {
    ...
    data: () => ({
        apiUrl: 'https://localhost:44398/FileSystem',
        items: [],
        active: [],
        searchTerm: null,
        directory: null,
    ...
    methods: {
        ...
        checkDirectoryExists(dir) {
            ...
            axios.get(`${this.apiUrl}/CheckDirectoryExists?fullName=${dir}`)
                .then(async ({ data }) => {
                    if (data)
                    {
                        if (this.directory) {
        ...
        fetchData(dir) {
            return new Promise((resolve, reject) => {
                axios.get(`${this.apiUrl}/?fullNmae=C:\\SinjulMSBH\\FileSystem&formatterName=Humanizer`)
                    .then(({ data }) => {
