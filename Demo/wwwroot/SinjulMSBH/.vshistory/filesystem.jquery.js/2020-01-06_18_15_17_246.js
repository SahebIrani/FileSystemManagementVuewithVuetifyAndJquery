                ...
                if (status === 'success') {
                    ...
                    if (obj.children('div').length > 0) {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            ...
                            var strTabStyle = `margin-left:${tab}px;margin-top:10px;cursor:pointer;`;
                            ...
                            if (data[i].isFile) {
                                ...
                                let location = `${dir}\\${name}`;
                                location = location.replace(new RegExp(location, '\\'), '@');

                                const fileResult = `<div id=${id}${index} class="${strTabClass}" style="${strTabStyle}">
                                                        <span style="${strTabStyle}">
                                                         <a class="btn btn-link text-light" href="/OpenFile/${location}">
                                ...
                                const folderResult = `<div id=${id}${index} class="${strTabClass} border p-2" style="${strTabStyle}">
